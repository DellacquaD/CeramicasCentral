...
export function useProducto(slug) {
    ...
    const producto = computed(() => store.getProductoBySlug(typeof slug === 'string' ? slug : slug.value));
    const productoFormateado = computed(() => producto.value ? store.transformarProducto(producto.value) : undefined);
    const cargarSiEsNecesario = async () => {
        if (!store.initialized) {
            await store.cargarProductos();
        }
 ...
 */
export function useCategoria(categoria) {
    const store = useProductsStore();
    ...
    const categoriaInfo = computed(() => {
        const cat = typeof categoria === 'string' ? categoria : categoria.value;
        return store.categories.find(c => c.slug.toLowerCase() === cat.toLowerCase() ||
 ...
 */
export function formatearPrecio(precio, moneda = 'UYU') {
    const simbolos = {
        UYU: '$',
        USD: 'US$',
        EUR: '€'
 ...
 */
export function calcularDescuento(precioActual, precioAnterior) {
...
export function filtrarPorPrecio(productos, precioMin, precioMax) {
    return productos.filter(p => {
        if (precioMin !== undefined && p.precio < precioMin)
            return false;
        if (precioMax !== undefined && p.precio > precioMax)
            return false;
        return true;
    });
}
 ...
 */
export function obtenerProductosRelacionados(producto, todosLosProductos, limite = 4) {
    const relacionados = todosLosProductos.filter(p => {
        // No incluir el mismo producto
        if (p.id === producto.id)
            return false;
        // Verificar si comparten material
        if (producto.material_id && p.material_id === producto.material_id)
            return true;
        // Verificar si comparten alguna categoría
        if (producto.categories.some(cat => p.categories.some(pCat => pCat.id === cat.id)))
            return true;
        // Verificar si comparten color
        if (producto.color_id && p.color_id === producto.color_id)
            return true;
        return false;
    });
 ...
 */
export function verificarDisponibilidad(producto, cantidadSolicitada = 1) {
 ...
 */
export function generarMetaSEO(producto) {
